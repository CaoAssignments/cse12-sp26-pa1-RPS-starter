...
**Due Date: Thursday, April 9, 2026, 11:59 pm PST**
...
**Note:** this is the same pledge mentioned in the Syllabus on page 9. If you have already completed this then you do not have to do it again.
...
* `public void playRPS(String playerMove, String cpuMove)`: Play one game of RPS using the moves provided.  This method should:
    ...
    * Print an appropriate message as shown above (e.g. `I chose scissors. You win.`).  <ins>Please use the provided static member string variables provided in `RPSAbstract.java` for this purpose</ins> (Hint: You will find the static method [System.out.printf()](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/io/PrintStream.html#printf(java.lang.String,java.lang.Object...)) useful, as well as this [documentation](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/util/Formatter.html#syntax) on *Format* strings).  
...
* `public void populateRecentWins()`: Update the `recentPlayerMoves` and `recentCPUMoves` arrays with the moves corresponding to the player's last 3 wins (in order of most recent to least recent).
    * **DO NOT** call this method anywhere in your code. The only place you should be updating the `recentPlayerMoves` and `recentCPUMoves` arrays is inside of `populateRecentWins()`. 
    * If the player has less than 3 wins, you should store as many of the recent wins as possible and leave the rest of the array empty.
    * You should use the appropriate member variables (`cpuMoves`, and `playerMoves`) to identify the game history.
...
* `public int determineWinner(String playerMove, String cpuMove)`: Given the two moves, determine the outcome of the game (-1 for invalid input, 0 for tie, 1 for player win, and 2 for cpu win).  But this is not as simple as “rock beats scissors”, “scissors beats paper” and “paper beats rock”!
    * Here is how you should determine the outcome:  All of the possible moves are stored in the array `possibleMoves`.  Each element in the array beats the **previous** element in the array, and the beginning wraps around to the end. All other pairings lead to a tie. This means each move beats one move and loses to one move. For example, in the move set  `{ "elephant", "alligator", "hedgehog", "mouse" }` mouse beats hedgehog, hedgehog beats alligator, alligator beats elephant, and elephant beats mouse (all other pairings tie).  <ins>Your code should be able to handle **any** array of possible moves with at least 3 elements.</ins>
...
* `public static void main(String[] args)`: main method that reads user input, generates CPU moves, and plays the game. This method is partially completed, you are to fill in the rest.
    ...
    * If the player enters `q`, then the game should end and the system should print out up to the last 10 games, in reverse order. The system should also print the player's last 3 winning games. If 10 games have not been played, it should print out as many games as has been played. The same idea applies for the last 3 winning games. Use the provided `displayStats()` method to print the win and tie statistics as shown in the example. As long as you update the member variables as described in `playRPS()` and `populateRecentWins()`, then a call to `displayStats()` should display the statistics correctly.
    ...
    * Call `genCPUMove()` once each time you prompt the player for a new move. <ins>Even if the previous player move is invalid, generate a new move for the CPU.</ins>
